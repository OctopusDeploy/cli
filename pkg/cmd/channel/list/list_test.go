...
func TestChannelList(t *testing.T) {
	...
	tests := []struct {
		...
		{"channel list picks up project from flag and filters by name", func(t *testing.T, api *testutil.MockHttpServer, qa *testutil.AskMocker, rootCmd *cobra.Command, stdOut *bytes.Buffer, stdErr *bytes.Buffer) {
			cmdReceiver := testutil.GoBegin2(func() (*cobra.Command, error) {
				defer api.Close()
				rootCmd.SetArgs([]string{"channel", "list", "-p", "Projects-22", "--filter", "hot", "--no-prompt", "-f", "table"})
				return rootCmd.ExecuteC()
			})
		...
		{"channel list prints only the header when the filter matches no channels", func(t *testing.T, api *testutil.MockHttpServer, qa *testutil.AskMocker, rootCmd *cobra.Command, stdOut *bytes.Buffer, stdErr *bytes.Buffer) {
			cmdReceiver := testutil.GoBegin2(func() (*cobra.Command, error) {
				defer api.Close()
				rootCmd.SetArgs([]string{"channel", "list", "-p", "Projects-22", "-q", "nomatch", "--no-prompt", "-f", "table"})
				return rootCmd.ExecuteC()
			})
