...
func (s *Select) filterOptions(config *survey.PromptConfig) []core.OptionAnswer {
	...
	if s.filter == "" {
		return append(core.OptionAnswerList(s.Options), core.OptionAnswer{
			Index: -1,
			Value: constants.PromptCreateNew,
		})
	}
...
func (s *Select) Prompt(config *survey.PromptConfig) (interface{}, error) {
	...
	// figure out the options and index to render
	opts, idx := paginate(pageSize, append(core.OptionAnswerList(s.Options), core.OptionAnswer{
		Index: -1,
		Value: constants.PromptCreateNew,
	}), s.selectedIndex)
