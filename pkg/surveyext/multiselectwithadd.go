...
for them to select using the arrow keys and enter, typing allows the user to add new entries not in the list. Response type is a slice of strings.
...
func (m *MultiSelectWithAdd) OnChange(key rune, config *survey.PromptConfig) {
	...
	if key == terminal.KeyArrowUp || (m.VimMode && key == 'k') {
		...
		if len(m.filter) > 0 {
			m.filter += string(terminal.KeySpace)
		} else if m.selectedIndex < len(options) {
...
func (m *MultiSelectWithAdd) filterOptions(config *survey.PromptConfig) []core.OptionAnswer {
	// the filtered list
